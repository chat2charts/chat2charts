...
import { saveProjectApi, fetchProjectApi, uploadFile, updateProjectApi } from '@/api/prolist'
// 画布枚举
...
// 虚拟数据
// import res from './virsualData.json'
...
export const useSync = () => {
  ...
  const dataSyncFetch = async () => {
    ...
    try {
      ...
      console.log(fetchRouteParamsLocation(),'fetchRouteParamsLocation');
      
      // 自定义ID版
      // const res = await fetchProjectApi({ projectId: '1805869230548639745' })
      ...
      if (res && res.code === ResultEnum.SUCCESS) {
        if (res.data) {
          ...
          // chartEditStore.setProjectInfo(ProjectInfoEnum.PROJECT_ID, '1805869230548639745')
        }
        setTimeout(() => {
  ...
  const dataSyncUpdate = throttle(async (updateImg = true) => {
    ...
    try {
      if (updateImg) {
        ...
        // 生成图片
        let scale = import.meta.env.VITE_THUMBNAIL_SCALE
        ...
        const uploadRes = await uploadFile(uploadParams) as any
        // 保存预览图
        console.log(fetchRouteParamsLocation(),'fetchRouteParamsLocation(),');
        ...
        if(uploadRes && uploadRes.code === ResultEnum.SUCCESS) {
          if (uploadRes.data.fileurl) {
            await updateProjectApi({
              ...
              updateUser:systemStore.userInfo.userName
            })
          } else {
            await updateProjectApi({
              id: fetchRouteParamsLocation(),
    ...
    params.append('updateUser',systemStore.userInfo.userName as any)
