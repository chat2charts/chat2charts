...
import { oauthLogin } from '@/api/sqlserver'
...
import { setToken, setToken2 ,setToken3 } from '@/utils/auth'
...
const handleSubmit = async (e: Event) => {
  ...
  formRef.value.validate(async (errors: any) => {
    if (!errors) {
      ...
      const resLogin = await oauthLogin(obj)

      // console.log(resLogin.data,'resLogin');
      


      // 连接ip登录，二者取其一
      setToken('DD7455950548DAB568D93033F27858B3')
      setToken2('2')
      setToken3(resLogin.data.data)
      setLocalStorage(
          GO_LOGIN_INFO_STORE,
          cryptoEncode(
            JSON.stringify({
